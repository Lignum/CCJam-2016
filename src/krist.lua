...
function Krist.makeAddress(pkey)
	local requestBody = {
		privatekey = sha256("KRISTWALLET" .. pkey) .. "-000"
	}

	local req = http.post(Krist.getNode(true) .. "/v2", JSON.encode(requestBody), {
	...
	local respjson = JSON.decode(res)
...
function Krist.login(pkey)
	local requestBody = {
		privatekey = sha256("KRISTWALLET" .. pkey) .. "-000"
	}

	local req = http.post(Krist.getNode(true) .. "/login?v=2", JSON.encode(requestBody), {
		["Content-Type"] = "application/json"
	})
	...
	if respjson.ok and respjson.authed then
		return respjson.address, requestBody.privatekey
	end
end

function Krist.performTX(privatekey, to, amt)
	local requestBody = {
	...
	local req = http.post(Krist.getNode(true) .. "/transactions", JSON.encode(requestBody), {
		["Content-Type"] = "application/json"
	})
